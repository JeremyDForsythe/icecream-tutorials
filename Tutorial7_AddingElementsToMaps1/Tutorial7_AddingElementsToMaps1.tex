...
\kulbox{\textbf{NOTE:} You now have two windows of QGIS open, what I am referring to here as the main one is the QGIS window that you have been working with so far. It has the panel that displays your basemap and any data layers you have loaded in, it has the browser panel and the layers panel. The second window is the \textit{Print Layout} window that we are making our map in. The two windows communicate with each other. For instance, if you were to scroll and zoom around in the main panel of the QGIS window the map we just rendered in the \textit{Print Layout} window would update to that view, unless we lock it in the Item window. More on this ahead...}
...
5. Adjust the \textit{Scale}, which is the zoom level, to 1200000 and press Enter.

\kulbox{\textbf{NOTE:} I have pre-selected this number for you based on my own trial and error and found that 1200000 framed the map nicely. If you disagree, feel free to play around with this setting and find one that suits your personal style. At the end of the day these maps are a representation of you and we want them to reflect that. However, they also need to effectively communicate the data to your audience so make sure you are able to explain how your design choices do that!} 
